(function() {
  ...
  (_base = window.localStorage).displayedTimers || (_base.displayedTimers = 'null');

}).call(this);
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/checkup.map
;
/*!
...
//# sourceMappingURL=/assets/source_maps/bundler/gems/turbolinks-2.2.1/lib/assets/javascripts/turbolinks.map
;
/* ========================================================================
...
(function() {
  ...
  CheckUp.Category = (function() {
    function Category(railsCategory) {
      var key, value;
      for (key in railsCategory) {
        if (!__hasProp.call(railsCategory, key)) continue;
        value = railsCategory[key];
        this[key] = value;
      }
    }

    Category.getCategoryColor = function() {
      return $.ajax({
        url: "/setup/color",
        type: "GET",
        dataType: 'json'
        ...
        return CheckUp.Category.setSetupCategoryDivColors();
      });
    };

    Category.defaultColors = ["41,128,185", "39,174,96", "241,196,15", "211,84,0", "192,57,43"];

    Category.setSetupCategoryDivColors = function() {
      var $categoryDiv, categoryTitle, color, takenColors, _ref;
      takenColors = [];
      _ref = CheckUp.Category.colors;
      for (categoryTitle in _ref) {
        color = _ref[categoryTitle];
      ...
      return CheckUp.Category.defaultColors = $(CheckUp.Category.defaultColors).not(takenColors).get();
    };

    Category.categoryRequest = function(attrs, type, callback) {
      if (attrs == null) {
        attrs = {};
      }
      return $.ajax({
    ...
    Category.categoryClicked = function(event) {
      event.preventDefault();
    ...
    Category.deactivateCategory = function($categoryNode) {
      var callback, categoryId, oneCategoryLeft;
      categoryId = $categoryNode.attr("data-category-id");
      callback = function(response) {
        var $deletedCategory;
        $deletedCategory = $("[data-category-id='" + response.id + "']");
        CheckUp.Category.defaultColors.push($deletedCategory.css("background-color").slice(4, -1).split(" ").join(""));
        return $deletedCategory.remove();
      };
      oneCategoryLeft = function() {
      ...
      if (!oneCategoryLeft()) {
        return CheckUp.Category.categoryRequest({
          id: categoryId,
          active: false
        }, 'PATCH', callback);
      }
    ...
    Category.newCategoryClick = function() {
      var $allCategoryDivs, $categoryNameForm, callback;
      event.preventDefault();
      ...
      callback = function(response) {
        ...
        return CheckUp.Category.defaultColors.splice(0, 1);
      };
    ...
    Category.toggleDetails = function($eventTarget) {
      $eventTarget.siblings().fadeToggle();
      return false;
    };

    Category.prototype.renderNode = function() {
      var $addTagButton, $addTagForm, $categoryDiv, $deleteButton, $tagList;
      $categoryDiv = $('<div/>', {
      ...
      $categoryDiv.append($tagList).append($addTagForm).append($addTagButton).append($deleteButton);
      return $categoryDiv.click(CheckUp.Category.categoryClicked.bind($categoryDiv));
    };

    Category.prototype.attachSorted = function() {
      var $allCategoryDivs, $categoryTitles, index, inserted, textValue, _i, _ref;
      $categoryTitles = $('.category-title');
      $allCategoryDivs = $('[data-category-id]');
      inserted = false;
      textValue = function(index) {
        return $categoryTitles.eq(index).text();
      };
      for (index = _i = 0, _ref = $categoryTitles.length; 0 <= _ref ? _i <= _ref : _i >= _ref; index = 0 <= _ref ? ++_i : --_i) {
      ...
      if (!inserted) {
        return $allCategoryDivs.last().after(this.renderNode());
      }
    };
...
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/category.map
;
(function() {
  CheckUp.drawEvent = (function() {
    function drawEvent() {}

    drawEvent.drawCategoryBars = function() {
      var $allCategories, $category, $categoryEventTitle, $innerCategory, $singleLine, categoryArray, categoryTimes, duration, durationPercent, index, leftPosition, title, titleArray, totalTime, widthPercent, _i, _len;
      ...
      $singleLine = function() {
        return $('<div/>', {
          ...
          width: "100%",
          click: function(event) {
            return CheckUp.drawEvent.toggleTagBars(event);
          }
        });
      };
      totalTime = 0;
      categoryArray = [];
      titleArray = [];
      for (title in categoryTimes) {
        duration = categoryTimes[title];
        $innerCategory = $('<div/>', {
          "class": "category-inner-bar",
          id: "" + title,
          "data-duration": duration,
          css: {
            "background-color": CheckUp.Category.colors[title] ? "rgb(" + CheckUp.Category.colors[title] + ")" : "#95a5a6"
          }
        });
      ...
      widthPercent = 0;
      leftPosition = 0;
      for (index = _i = 0, _len = categoryArray.length; _i < _len; index = ++_i) {
      ...
      $('#master-event').empty();
      return $('#master-event').append($allCategories);
    };

    drawEvent.drawTagBars = function(categoryTitle) {
      var $categoryBreakdown, $categoryUl, $tagLi, categoryObj, opacity, tagName, tagPercentOfCategory, tagTime, totalCategoryTime;
      ...
      for (tagName in categoryObj) {
        tagTime = categoryObj[tagName];
        tagPercentOfCategory = (tagTime / totalCategoryTime) * 100;
        opacity = (tagPercentOfCategory / 200) + 0.6;
        $tagLi = $('<li/>', {
          id: "" + tagName,
          html: "<p class='ellipsis float-left'>" + tagName + "</p> <p class='float-right'>" + ((tagTime / 60).toFixed(1)) + "</p>",
          "class": 'tag-list-li',
          css: {
            width: "" + tagPercentOfCategory + "%",
            display: "inline-block",
            "background-color": !CheckUp.Category.colors[categoryTitle] ? "#95a5a6" : "rgba(" + CheckUp.Category.colors[categoryTitle] + "," + opacity + ")"
          },
          click: function(event) {
            var $overflow, thisHTML;
            thisHTML = $(this).html();
            $overflow = $(this).closest('ul').prev();
            if ($overflow.html() === thisHTML && !$overflow.is(":hidden")) {
              return $overflow.hide();
            } else {
              return $overflow.html("" + thisHTML).show();
            }
          }
        });
      ...
      return $categoryBreakdown.slideToggle();
    };

    drawEvent.toggleTagBars = function(event) {
      var $clickedCategoryTagBar, clickedCategoryTitle;
      clickedCategoryTitle = $(event.currentTarget).children('[id]').attr('id');
      if (clickedCategoryTitle === "untracked") {
        return false;
      }
      $clickedCategoryTagBar = $("#" + clickedCategoryTitle + "-tags");
      if (($clickedCategoryTagBar != null ? $clickedCategoryTagBar.length : void 0) > 0) {
        return $clickedCategoryTagBar.slideToggle();
      } else {
        return CheckUp.drawEvent.drawTagBars(clickedCategoryTitle);
      }
    };
...
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/drawEvent.map
;
(function() {
  CheckUp.Error = (function() {
    function Error() {}

    Error.drawError = function(message, classType) {
      $(classType).text(message);
      $(classType).fadeIn('fast');
      if (classType === '.notice') {
        return $(classType).delay(2000).fadeOut('slow');
      } else {
        return $(classType).delay(3000).fadeOut('slow');
      }
    };

    Error.eventErrorHandle = function(timeArray) {
      var bool;
      bool = true;
    ...
    Error.isCategoryValid = function($form, $existingCategories) {
      var bool;
      bool = false;
      if ($form.val().length >= 3 && $existingCategories.length < 5) {
        bool = true;
      ...
      } else if ($form.val().length > 0 && $form.val().length < 3) {
...
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/error.map
;
(function() {
  CheckUp.Event = (function() {
    function Event() {}

    Event.dateClick = function() {
      var end, errorResult, start, timeArray;
      ...
      return CheckUp.Event.categoryListEmpty();
    };

    Event.getEventRequest = function(start, end) {
      return $.ajax({
        url: "/events",
        type: 'GET',
        dataType: 'json',
        ...
        return $('#master-event').prepend($('<h3/>', {
          "class": 'time-period-title',
          text: "" + (CheckUp.Event.timePeriod[0].toString().slice(0, 10)) + " to " + (CheckUp.Event.timePeriod[1].toString().slice(0, 10))
        }));
      });
    };

    Event.dateClick = function() {
      var end, start, timeArray;
      CheckUp.Event.reloadInit();
      timeArray = CheckUp.Event.getStartAndEndTime();
      if (timeArray.length === 2) {
        start = timeArray[0].getTime();
    ...
    Event.makeEventBreakdown = function(structure) {
      var categoryTitle, eventBreakdown, eventItem, eventsArray, tagDuration, tagName, tagObject, _i, _len;
      eventBreakdown = {};
      for (categoryTitle in structure) {
        tagObject = structure[categoryTitle];
        eventBreakdown[categoryTitle] = eventBreakdown[categoryTitle] || {};
        for (tagName in tagObject) {
          eventsArray = tagObject[tagName];
          tagDuration = 0;
          for (_i = 0, _len = eventsArray.length; _i < _len; _i++) {
            eventItem = eventsArray[_i];
    ...
    Event.makecategoryTimes = function(structure, viewStart, viewEnd) {
      var categoryDuration, categoryTimes, categoryTitle, duration, endTime, eventBreakdown, startTime, tagDuration, tagName, tagsObject, timeDiff, totalDuration;
      categoryTimes = {};
      totalDuration = 0;
      eventBreakdown = CheckUp.Event.makeEventBreakdown(structure);
      for (categoryTitle in eventBreakdown) {
        tagsObject = eventBreakdown[categoryTitle];
        categoryTimes[categoryTitle] = categoryTimes[categoryTitle] || 0;
        categoryDuration = 0;
        for (tagName in tagsObject) {
          tagDuration = tagsObject[tagName];
    ...
    Event.renderCategoryTime = function(structure, start, end) {
      var category, categoryTime, day, dayDuration, days, duration, endTime, eventArray, i, startTime, tag, tagArray, tagObject, tagTime, timeAllCategory, timeDiff, _i, _len;
      categoryTime = 0;
      tagTime = 0;
      timeAllCategory = 0;
      eventArray = [];
      ...
      for (category in structure) {
        tagObject = structure[category];
        for (tag in tagObject) {
          tagArray = tagObject[tag];
          for (_i = 0, _len = tagArray.length; _i < _len; _i++) {
            eventArray = tagArray[_i];
      ...
      return $('#category-time').append("<li>Untracked Time: " + (duration - timeAllCategory) + " minutes, percentage: " + ((((duration - timeAllCategory) / duration) * 100).toFixed(2)) + "%</li>");
    };

    Event.renderTagTime = function(categoryIndex, category, tag, tagTime) {
      $(".category-" + categoryIndex + "-tag").append("<li>" + tag + " time: " + tagTime + " minutes<ol class='" + category + "_" + tag + "'></ol></li>");
      return 0;
    };

    Event.renderEventTime = function(tagArray, category, tag) {
      var eventArray, time, _i, _len, _results;
      _results = [];
      for (_i = 0, _len = tagArray.length; _i < _len; _i++) {
        eventArray = tagArray[_i];
        time = new Date(eventArray[0]);
        _results.push($("." + category + "_" + tag).append("<li>From " + (CheckUp.Event.renderTimeformat(time)) + " , duration is " + eventArray[2] + " minutes </li>"));
      }
      return _results;
    };

    Event.renderTimeformat = function(time) {
      var timeStr;
      timeStr = time.toString();
      return timeStr.split("GMT")[0];
    };

    Event.reloadInit = function() {
      var day, yesterday;
      day = new Date();
      ...
      return $("#category-time").append("<li class='category-4'><ul class='category-4-tag'></ul></li>");
    };

    Event.categoryListEmpty = function() {
      $("#from").val("");
      $("#to").val("");
      $('#cal-1').hide();
      return $('#cal-2').hide();
    ...
    Event.getStartAndEndTime = function() {
      var dateArrayEnd, dateArrayStart, dateEnd, dateStart, endArray, endInput, endTime, month, startInput, startTime, timeArray;
      timeArray = [];
      startInput = "";
      endInput = "";
    ...
    Event.setAttrMultiple = function() {
...
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/event.map
;
(function() {
  CheckUp.ready = function() {
    ...
    if ($('body').attr('data-action') === 'setup_page') {
      ...
      return $('#category-time-show').click(CheckUp.Event.dateClick);
      ...
      return $(".tag-category-btn").click(CheckUp.Tag.setTagColor);
...
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/onload_handlers.map
;
(function() {
  CheckUp.Routine = (function() {
    function Routine() {}

    Routine.appendTimers = function() {
      var $tagTimer, tagCurrentEventId, tagId, timerText, updatedDate, _ref, _results;
      CheckUp.Routine.DISPLAYED_TIMERS = JSON.parse(window.localStorage.displayedTimers) || {
        updatedAt: new Date()
      };
      updatedDate = new Date(CheckUp.Routine.DISPLAYED_TIMERS.updatedAt);
      if (new Date().getDate() === updatedDate.getDate()) {
        _ref = CheckUp.Routine.DISPLAYED_TIMERS;
        _results = [];
        for (tagId in _ref) {
          timerText = _ref[tagId];
          $tagTimer = $("[data-routine-tag-id='" + tagId + "']").find('.timer');
          tagCurrentEventId = $tagTimer.attr("data-routine-current-event-id");
          if (tagCurrentEventId == null) {
            _results.push(delete CheckUp.Routine.DISPLAYED_TIMERS[tagId]);
          } else if (parseInt(tagCurrentEventId) !== -1) {
            _results.push(CheckUp.Routine.eventRequest({
              id: tagCurrentEventId,
              timer_text: timerText
            }, function(response) {
              var $activeTag, $activeTimer, activeTagId, runTime;
              ...
              return CheckUp.Routine.toggleTimer($activeTag, activeTagId);
            }));
          } else {
            _results.push($tagTimer.text(timerText));
          }
        }
        return _results;
      }
    ...
    Routine.updateCache = function(tagId, timerText) {
      CheckUp.Routine.DISPLAYED_TIMERS[tagId] = timerText;
      CheckUp.Routine.DISPLAYED_TIMERS['updatedAt'] = new Date();
      return window.localStorage.displayedTimers = JSON.stringify(CheckUp.Routine.DISPLAYED_TIMERS);
    };

    Routine.eventRequest = function(attrs, callback) {
      if (attrs == null) {
        attrs = {};
      }
      return $.ajax({
    ...
    Routine.tagEventRequest = function(attrs, callback) {
      if (attrs == null) {
        attrs = {};
      }
      return $.ajax({
    ...
    Routine.routineClicked = function(event) {
      var $tagClicked, tagClickedId;
      ...
      if (tagClickedId) {
        ...
        return CheckUp.Routine.tagEventRequest({
          id: tagClickedId
        }, CheckUp.Routine.appendTimers);
      }
    };

    Routine.stringToMin = function(timerText) {
      var matches, minutes;
      matches = timerText.match(/(\d+)/g) || [0];
      return minutes = matches.length === 2 ? parseInt(matches[0] * 60) + parseInt(matches[1]) : parseInt(matches[0]);
    };

    Routine.minToString = function(startTime) {
      var hours, minutes;
    ...
    Routine.toggleTimer = function($tagClicked, tagClickedId) {
      var $activeTimer, $timer, startTime, tickTock;
      ...
      if ($activeTimer[0] !== $timer[0]) {
        ...
        tickTock = setInterval(function() {
          startTime += 1;
          $timer.text(CheckUp.Routine.minToString(startTime));
          return CheckUp.Routine.updateCache(tagClickedId, $timer.text());
        }, 60000);
        return $timer.attr('data-timer-id', tickTock);
      }
    };

    Routine.setRoutineNavWidth = function() {
      var $list, listLength;
      $list = $('.category-of-tags').remove();
      $list.insertAfter($('.nav-bar'));
      listLength = $list.children().length;
      return $('.category-of-tags li').css("width", "" + (100 / listLength) + "%");
    };
...
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/routine.map
;
(function() {
  var __hasProp = {}.hasOwnProperty;

  CheckUp.Tag = (function() {
    function Tag(railsTag) {
      var key, value;
      for (key in railsTag) {
        if (!__hasProp.call(railsTag, key)) continue;
        value = railsTag[key];
        this[key] = value;
      }
    }

    Tag.setTagColor = function() {
      var $matchedElements, defaultColor;
      ...
      if ($matchedElements.eq(0).css("background-color") === defaultColor) {
        return $("[data-routine-category-id='" + ($(this).attr("data-category-id")) + "']").css("background-color", $(this).css("background-color"));
      } else {
        return $("[data-routine-category-id='" + ($(this).attr("data-category-id")) + "']").css("background-color", defaultColor);
      }
    };

    Tag.tagRequest = function(attrs, type, callback) {
      if (attrs == null) {
        attrs = {};
      }
      return $.ajax({
    ...
    Tag.routineClicked = function(event) {
      event.preventDefault();
    ...
    Tag.deactivateTag = function($eventTarget) {
      var $tagLi, callback, tagId;
      $tagLi = $eventTarget.parents('[data-tag-id]');
      tagId = $tagLi.attr("data-tag-id");
      callback = function(response) {
        $("[data-tag-id='" + response.id + "']").remove();
        return $("[data-routine-tag-id='" + response.id + "']").remove();
      };
      return CheckUp.Tag.tagRequest({
        id: tagId,
        active: false
      }, 'PATCH', callback);
    };

    Tag.createTag = function($category) {
      var $categoryId, $tagNameForm, callback, isTagValid;
      ...
      callback = function(response) {
        ...
        return $tagNameForm.val("");
      };
      isTagValid = function($form) {
        return $form.val().length >= 3;
      };
    ...
    Tag.addToRoutine = function($eventTarget) {
      var $routineList, $tagLi, callback, tagId;
      $tagLi = $eventTarget.parents('[data-tag-id]');
      tagId = $tagLi.attr('data-tag-id');
      ...
      callback = function(response) {
        ...
        return $("[data-tag-id='" + response.id + "']").find('.routine-add-tag-btn').remove();
      };
      return CheckUp.Tag.tagRequest({
        id: tagId,
        routine: $routineList.length
      }, 'PATCH', callback);
    };

    Tag.removeFromRoutine = function($eventTarget) {
      var $addRoutineButton, $tagLi, callback, tagId;
      ...
      callback = function(response) {
        ...
        return $("[data-tag-id='" + response.id + "']").prepend($addRoutineButton);
      };
      return CheckUp.Tag.tagRequest({
        id: tagId,
        routine: -1
    ...
    Tag.saveFullRoutine = function() {
      var $routine, domLi, index, tagIdsOrderedByRoutine, _i, _len;
      $routine = $('[data-routine-tag-id]');
      tagIdsOrderedByRoutine = [];
      for (index = _i = 0, _len = $routine.length; _i < _len; index = ++_i) {
        domLi = $routine[index];
      ...
      return $.ajax({
        ...
        return CheckUp.Error.drawError(response.status, '.notice');
      }).fail(function(error) {
        return console.log(error);
      });
    };

    Tag.prototype.attachToCategory = function() {
      var $addRoutineButton, $categoryTagListItems, $deleteButton, $tagLi, $tagNames, $thisCategory, index, inserted, textValue, _i, _ref;
      $tagLi = $('<li/>', {
        'data-tag-id': this.id,
      ...
      inserted = false;
      textValue = function(index) {
        return $tagNames.eq(index).text();
      };
      if ($tagNames.length === 0) {
        return $thisCategory.find('.category-tag-list').append($tagLi);
      } else {
        for (index = _i = 0, _ref = $tagNames.length; 0 <= _ref ? _i <= _ref : _i >= _ref; index = 0 <= _ref ? ++_i : --_i) {
        ...
        if (!inserted) {
          return $categoryTagListItems.last().after($tagLi);
        }
      }
    };

    Tag.prototype.attachToRoutine = function() {
      var $removeButton, $routineUl, $tagLi;
      ...
      return $tagLi.prepend($removeButton).appendTo($routineUl);
    };
...
//# sourceMappingURL=/assets/source_maps/app/assets/javascripts/tag.map
;
