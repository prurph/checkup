...
describe Event do
  ...
  before(:each) do
    ...
    @category = Category.create(user: @user, title: "Personal", active: true)
  ...
  it 'should calculate event duration where event is within the view period' do
    @event = Event.create(tag: @tag, created_at: Time.utc(2014, 03, 04, 5, 0, 0), updated_at: Time.utc(2014, 03, 04, 7, 0, 0))
  ...
  it 'should calculate event duration where event starts outside the view period and ends inside the view period' do
    @event = Event.create(tag: @tag, created_at: Time.utc(2014, 03, 03, 22, 0, 0), updated_at: Time.utc(2014, 03, 04, 5, 0, 0))
  ...
  it 'should calculate event duration where event starts inside the view period and ends outside the view period' do
    @event = Event.create(tag: @tag, created_at: Time.utc(2014, 03, 04, 20, 0, 0), updated_at: Time.utc(2014, 03, 05, 8, 0, 0))
  ...
  it 'should calculate event duration where event starts and ends outside the period' do
    @event = Event.create(tag: @tag, created_at: Time.utc(2014, 03, 03, 15, 0, 0), updated_at: Time.utc(2014, 03, 07, 12, 0, 0))
